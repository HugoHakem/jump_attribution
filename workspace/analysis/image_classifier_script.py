...
def plot_fake_img(fake_img_path_preffix, real_img_path, dataset_fold, channel=None, mode="ref", fold=0, split="train", use_ema=True,
                  num_img_per_domain=2, seed=42, preffix="image_active_", fig_directory=Path("./figures")):
    ...
    for i, label in enumerate(domains):
        ...
        for j, idx in enumerate(label_idx):
            ...
            for k, fk_idx in enumerate(fake_idx):
                ...
                axs[N * i + n * j + k][0].imshow(channel_to_rgb(real_img, channel, channel_last=True))
                if k == 0:
                    axs[N * i + n * j + k][0].set_title(f"Real image - idx: {idx} - Class_{label}")
                for l in range(fake_imgs.shape[0]):
                    axs[N * i + n * j + k][l+1].imshow(channel_to_rgb(fake_imgs[l], channel, channel_last=True))
                    if l == 0:
...
# VGG_path = "VGG_image_active_fold_0epoch=78-train_acc=0.96-val_acc=0.91.ckpt" for 448 image size
VGG_path = "VGG_image_crop_active_fold_0_epoch=61-train_acc=0.95-val_acc=0.82.ckpt" # for 128 image size
...
# torch.set_float32_matmul_precision('medium') #try 'high')
# seed_everything(42, workers=True)
# tb_logger = pl_loggers.TensorBoardLogger(save_dir=Path("logs"), name="VGG_image_crop_active_test")
...
# trainer.logger = pl_loggers.TensorBoardLogger(save_dir=Path("logs"), name="VGG_image_crop_active_test")
...
def compute_fid_lpips(fake_img_path_preffix,  dataset_fold, mode="ref",
    ...
    # torchmetrics
    fid_metric = FrechetInceptionDistance(feature=fid_feature, reset_real_features=True, normalize=True,
                                          input_img_size=tuple(dataset[0][0].shape[-3:])).to(device)
...
"""
## Doesn't work need to transform img into RGB ! (here I tried to use a custom fid embedding_model but doesn't work because of cuda device.)
